...
static PANEL_ID: LazyLock<widget::Id> =
    LazyLock::new(|| widget::Id::new("grok-monitor-panel"));

const PANEL_ICON: &[u8] = include_bytes!(
    "../res/icons/hicolor/scalable/apps/io.github.simple-systems-se.grok-mon.svg"
);
...
impl cosmic::Application for GrokMonitor {
    ...
    fn update(&mut self, message: Self::Message) -> Task<cosmic::Action<Self::Message>> {
        match message {
            ...
            Message::OpenUsage => {
                let _ = std::process::Command::new("xdg-open").arg(USAGE_URL).spawn();
            }
            Message::CopyPercent => {
                if let Some(snapshot) = &self.snapshot {
                    return cosmic::iced::clipboard::write(format_percent(snapshot.percent));
                }
            }
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        ...
        let icon = widget::icon::from_svg_bytes(PANEL_ICON)
            .symbolic(true)
            .icon()
            .size(self.core.applet.suggested_size(true).0);

        let percent = self.core.applet.text(label).class(theme::Text::Color(color));

        let mut children: Vec<Element<'_, Message>> = vec![icon.into(), percent.into()];
...
impl GrokMonitor {
    ...
    fn chip_label(&self) -> (String, Color) {
        ...
        match (&self.snapshot, &self.error) {
            (Some(snapshot), err) => {
                ...
                if snapshot.percent >= f32::from(self.config.critical_percent) {
                    color = cosmic.destructive_color().into();
                } else if snapshot.percent >= f32::from(self.config.warn_percent) {
                    color = cosmic.warning_color().into();
                }
                if err.is_some() {
                    color.a *= 0.7;
                }
                (format_percent(snapshot.percent), color)
            }
    ...
    fn overview(&self) -> Element<'_, Message> {
        ...
        col = col.push(padded(column::with_capacity(2).push(text::title4(title)).push(
            text::caption(email),
        )));
        ...
        if let Some(snapshot) = &self.snapshot {
            col = col.push(padded(self.gauge(snapshot.percent)));

            let mut detail = format!(
                "{} · {}",
                period_label(snapshot.period_type.as_deref()),
                format_percent(snapshot.percent)
            );
        ...
        if let Some(error) = &self.error {
            col = col.push(padded(
                text::caption(error.to_string()).class(theme::Text::Color({
    ...
    fn settings_page(&self) -> Element<'_, Message> {
        let mut col = column::with_capacity(10).padding([12, 0]).spacing(8);
        col = col.push(padded(
            button::standard("← Back").on_press(Message::Back),
        ));
        ...
        col = col.push(padded(text::caption(format!(
            "Warn at {}% · Critical at {}%",
            self.config.warn_percent, self.config.critical_percent
        ))));
